...
const browser=await chromium.launch({headless:true});const baselines=[];
try{
 for(const viewport of [{width:390,height:844},{width:393,height:852},{width:430,height:932}]){
  ...
  const start=performance.now();await page.goto(origin+'/admin',{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>window.__crmOwnerView&&document.querySelector('#crmOwnerNavCustomers')&&document.querySelector('#crmMktList tbody tr'));const interactive=Math.round((performance.now()-start)*10)/10;
  await page.waitForTimeout(250);
  const beforeGets=customerGets,beforeFetch=totalFetches;const tapStart=performance.now();await page.locator('#crmOwnerNavCustomers').click();await page.waitForFunction(()=>document.body.dataset.crmOwnerView==='customers'&&document.querySelector('#crmOwnerNavCustomers')?.classList.contains('active'));await page.locator('#crmMktList.open').waitFor();const tapMs=Math.round((performance.now()-tapStart)*10)/10;
  ...
  const row={viewport:`${viewport.width}x${viewport.height}`,tapMs,interactive,customerApiGetPerTap:customerGets-beforeGets,totalFetchDelta:totalFetches-beforeFetch,callbacks:snapshot.callbacks,records:snapshot.records,fullDomScans:snapshot.callbacks+2,navBindCount:snapshot.callbacks+1,nodes:snapshot.nodes,...snapshot};baselines.push(row);
  console.log(`BASE_${viewport.width}_NAV_ACTIVE=${snapshot.navActive}`);console.log(`BASE_${viewport.width}_CUSTOMER_LIST_VISIBLE=${snapshot.listVisible}`);console.log(`BASE_${viewport.width}_CUSTOMER_EXCLUSIVE=${snapshot.dashboardVisible===0&&snapshot.todayVisible===0&&snapshot.deliveryVisible===0?1:0}`);console.log(`BASE_${viewport.width}_GROWTH_PANEL_VISIBLE=${snapshot.dashboardVisible}`);console.log(`BASE_${viewport.width}_CUSTOMER_TAP_TO_VISIBLE_MS=${tapMs}`);console.log(`BASE_${viewport.width}_OWNER_NAV_INTERACTIVE_MS=${interactive}`);console.log(`BASE_${viewport.width}_CUSTOMER_API_GET_PER_TAP=${row.customerApiGetPerTap}`);console.log(`BASE_${viewport.width}_TOTAL_FETCH_DELTA_PER_TAP=${row.totalFetchDelta}`);console.log(`BASE_${viewport.width}_MUTATION_CALLBACKS=${snapshot.callbacks}`);console.log(`BASE_${viewport.width}_MUTATION_RECORDS=${snapshot.records}`);console.log(`BASE_${viewport.width}_FULL_DOM_SCANS_SOURCE_DERIVED=${row.fullDomScans}`);console.log(`BASE_${viewport.width}_NAV_BIND_COUNT_SOURCE_DERIVED=${row.navBindCount}`);console.log(`BASE_${viewport.width}_DOM_NODES=${snapshot.nodes}`);
  assert(snapshot.navActive===1,'BASE RED invalid: Customer nav did not become active');assert(snapshot.listVisible===1,'BASE RED invalid: Customer list not visible');assert(snapshot.headingVisible===1&&snapshot.searchVisible===1,'BASE RED invalid: Customer primary controls not visible');assert(tapMs<=250,'Customer state switch too slow '+tapMs+'ms');assert(row.customerApiGetPerTap===0,'one tab tap caused duplicate Customer list fetch');assert(snapshot.overflow<=1,'horizontal overflow '+snapshot.overflow);assert(consoleErrors.length===0,'console errors '+consoleErrors.join(' | '));assert(pageErrors.length===0,'page errors '+pageErrors.join(' | '));
  await context.close();
 }
 console.log('BASE_METRICS='+JSON.stringify(baselines));
 assert(baselines.every(x=>x.dashboardVisible===0),'REAL_DEVICE_FALSE_POSITIVE_REPRODUCED: nav active + Customer list visible but #crmGrowthPanel / 総合ダッシュボード remains visible');
