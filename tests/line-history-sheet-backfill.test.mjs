...
assert.match(planner,/WHERE EXISTS \(SELECT 1 FROM customers/);
...
fs.writeFileSync(lineFile,lineCsv);fs.writeFileSync(masterFile,masterCsv);
...
assert.equal(summary.candidate_rows,3);
