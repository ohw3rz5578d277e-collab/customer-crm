...
const entry=fs.readFileSync('src/production-index-crm-line-history-backfill-entry.js','utf8');
...
assert.match(entry,/handleLineHistorySheetBackfill/);
assert.match(entry,/enforceProductionRequestBoundary/);
assert.match(entry,/ownerPasswordRequestAuthenticated/);
assert.match(wrangler,/production-index-crm-line-history-backfill-entry\.js/);
...
console.log('LINE_HISTORY_SHEET_WRITE_GATE=PASS');
