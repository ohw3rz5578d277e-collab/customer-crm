...
const staleEightDigitLine=uid('4');
...
const plannerLineCsv=lineCsv+'\n'+row(['2026/03/01 10:00:00','C342ee0c10d','みずき','古いC IDでもLINE IDで復元','text','FALSE','User',oldIdLine,'webhook','legacy-c-line-key','','','2026/03/01 10:00:01'])+'\n'+row(['2026/03/02 10:00:00','25999999','旧番号顧客','存在しない8桁IDは一意LINE IDへフォールバック','text','FALSE','User',staleEightDigitLine,'webhook','stale-8digit-line-key','','','2026/03/02 10:00:01'])+'\n'+row(['2026/01/14 10:00:00','','現代顧客','こんにちは','text','FALSE','User',modernLine,'webhook','modern-semantic-duplicate','','','2026/01/14 10:00:03']);
...
assert.equal(summary.planner_version,4);
assert.equal(summary.candidate_rows,6);
assert.equal(summary.batch_duplicate_rows,1);
assert.equal(summary.planned_insert_candidate_rows,5);
...
assert.equal(summary.safety.stale_customer_id_fallback_requires_nonexistent_hint,true);
assert.equal(summary.safety.batch_duplicate_candidates_skipped,true);
...
assert.match(previewSql,/batch_duplicate_rows/);
assert.match(previewSql,/batch_duplicate=0/);
assert.match(commitSql,/INSERT OR IGNORE INTO customer_line_messages/);
assert.equal((commitSql.match(/INSERT OR IGNORE INTO customer_line_messages/g)||[]).length,5);
assert.doesNotMatch(commitSql,/modern-semantic-duplicate/);
assert.match(commitSql,/SELECT COUNT\(\*\) FROM customers/);
assert.match(commitSql,new RegExp(oldIdLine));
assert.match(previewSql,/NOT EXISTS\(SELECT 1 FROM customers cx WHERE cx\.customer_id=candidates\.customer_id_hint\)/);
assert.match(commitSql,new RegExp(staleEightDigitLine));
assert.match(commitSql,/NOT EXISTS\(SELECT 1 FROM customers cx WHERE cx\.customer_id='25999999'\)/);
...
console.log('LINE_HISTORY_SHEET_STALE_8DIGIT_ID_LINE_USER_FALLBACK=PASS');
...
console.log('LINE_HISTORY_SHEET_BATCH_SEMANTIC_DUPLICATE=PASS');
