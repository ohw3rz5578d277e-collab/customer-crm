...
function analytics(u){
  ...
  return {ok:true,available:true,period:{from,to,as_of:'2026-09-05',span_days:5,previous:{from:'2026-08-27',to:'2026-08-31'}},current:{from,to,revenue,completed_shoots:3,unique_customers:2,average_order_value:Math.round(revenue/3),repeat_customers_in_period:1,repeat_rate_pct:50,line_additions:7,genres:[{genre:'七五三',shoots:2,revenue:Math.round(revenue*2/3),unique_customers:1},{genre:'お宮参り',shoots:1,revenue:Math.round(revenue/3),unique_customers:1}],monthly:[{month:from.slice(0,7),shoots:3,revenue,unique_customers:2}]},previous:{revenue:30000,completed_shoots:2,unique_customers:2,average_order_value:15000,line_additions:5},change_pct:{revenue:50,completed_shoots:50,unique_customers:0,average_order_value:0,line_additions:40},meta:{read_only:true,identity_key:'customer_id',customer_id_generation:false,customer_write:false,line_send:false}};
}
const server=http.createServer((req,res)=>{const u=new URL(req.url,'http://127.0.0.1');requests.push(req.method+' '+u.pathname+u.search);if(!['GET','HEAD'].includes(req.method))writes.push(req.method+' '+u.pathname);if(u.pathname==='/'||u.pathname==='/admin')return send(res,200,html,'text/html; charset=utf-8');if(u.pathname==='/api/customer360/marketing-home')return send(res,200,{ok:true,kpis:{customers:1,average_realized_ltv:50000,repeat_rate_pct:100,vip_high_ltv:0,event_90d:1,dormant_180:0,line_link_rate_pct:100,line_additions_this_month:7,approach_this_month:1},top_opportunities:[item],facets});if(u.pathname==='/api/customer360/analytics'){const from=u.searchParams.get('from')||'';const delay=from==='2026-07-01'?140:from==='2026-08-01'?10:0;if(delay)return setTimeout(()=>send(res,200,analytics(u)),delay);return send(res,200,analytics(u));}if(u.pathname==='/api/customer360/customers')return send(res,200,{ok:true,total:1,all_total:1,page:1,page_size:50,has_next:false,items:[item],facets,meta:{privacy_safe_list_dto:true}});if(u.pathname.startsWith('/api/customer360/customer/'))return send(res,200,{ok:true,customer:{...item,address:{},family:[],opportunities:[],reservations:[],line_history:[],marketing_history:[],marketing_classes:[],consent:{},recommendation:{}}});return send(res,404,{ok:false})});
...
try{
  for(const viewport of [{width:390,height:844},{width:1440,height:900}]){
    ...
    assert.ok((await page.locator('#crmMktHome').innerText()).includes('今月LINE追加'),'monthly LINE additions KPI missing');
    assert.ok((await page.locator('#crmMktHome').innerText()).includes('LINE追加 +40%'),'LINE additions period KPI missing');
  ...
  console.log('CUSTOMER360_LINE_ADDITIONS_ANALYTICS=PASS');
