...
assert(entry.includes('injectMobileOwnerCardSummary'),'mobile card summary production wiring missing');
assert(!entry.includes("from './crm-mobile-owner-interaction-recovery.mjs'"),'legacy recovery must not be imported by Production entry');
assert(!entry.includes('injectMobileOwnerInteractionRecovery('),'legacy recovery must not be composed into Production HTML');
assert(entry.includes('injectOwnerLineChat(withDirectNavigation)'),'canonical LINE chat must replace legacy LINE routing');
...
console.log('CRM_MOBILE_OWNER_INTERACTION_LEGACY_STATIC=PASS');
