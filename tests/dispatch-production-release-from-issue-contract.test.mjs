...
function parseCommand(body){
  ...
  if(m)return {mode:'preflight',sha:m[1],confirmProduction:false};
  m=body.match(deploy);
  if(m)return {mode:'deploy',sha:m[1],confirmProduction:true};
  return null;
}
...
assert.match(source,/deploy_re='\^\/crm-production-deploy confirm_production=true sha=\(\[0-9a-f\]\{40\}\)\$'/,'deploy confirmation contract must remain exact');
