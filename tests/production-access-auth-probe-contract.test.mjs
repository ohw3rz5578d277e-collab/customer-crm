...
assert.ok(probe.includes('group: customer-crm-production-access-auth-probe'),'probe must use an isolated concurrency group');
assert.doesNotMatch(probe,/group: customer-crm-production-deploy/,'probe must not occupy or replace the Production deploy pending slot');
assert.ok(probe.includes('actions: read'),'probe must have read-only Actions permission for deploy overlap checks');
assert.ok(probe.includes('Snapshot Production deploy activity'),'pre-probe deploy snapshot missing');
assert.ok(probe.includes('PRODUCTION_DEPLOY_ACTIVE_BEFORE_PROBE'),'probe must fail if a deploy is active before probing');
assert.ok(probe.includes('PRODUCTION_DEPLOY_RUN_BEFORE'),'probe must record prior deploy run identity');
assert.ok(probe.includes('Reject Production deploy overlap during probe'),'post-probe deploy overlap guard missing');
assert.ok(probe.includes('PRODUCTION_DEPLOY_ACTIVE_DURING_PROBE'),'probe must fail if a deploy is active during probing');
assert.ok(probe.includes('PRODUCTION_DEPLOY_CHANGED_DURING_PROBE'),'probe must fail if deploy run identity changes during probing');
assert.ok(probe.includes('PRODUCTION_DEPLOY_OVERLAP_DURING_PROBE=0'),'deploy overlap zero evidence missing');
...
console.log('PRODUCTION_PROBE_CONCURRENCY_ISOLATION=PASS');
console.log('PRODUCTION_DEPLOY_OVERLAP_GUARD=PASS');
