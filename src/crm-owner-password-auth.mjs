...
function invalidModeResponse(request,env){return html(loginPage('認証モード設定が不正です。管理者設定を確認してください。',ownerLoginLocation(request,env)),503)}
export async function handleOwnerPasswordAuth(request,env){
  const url=new URL(request.url),mode=authMode(env),loginAction=ownerLoginLocation(request,env);
  if(url.pathname==='/__crm/owner-login'&&request.method==='GET'){
    if(mode==='invalid')return invalidModeResponse(request,env);
    if(mode==='access')return html(loginPage('現在はCloudflare Accessログインが有効です。',loginAction),503);
    if(!configured(env))return html(loginPage('パスワードログインはまだ有効化されていません。',loginAction),503);
    if(passwordMode(mode)&&!rateLimiterConfigured(env))return html(loginPage('パスワードログインのrate limit設定が不足しています。',loginAction),503);
    ...
    return html(loginPage('',loginAction));
  }
  if(url.pathname==='/__crm/owner-login'&&request.method==='POST'){
    ...
    if(!(await passwordMatches(password,text(env.CRM_OWNER_PASSWORD))))return html(loginPage('パスワードが違います。',loginAction),401);
  ...
  if(url.pathname==='/__crm/owner-logout'&&request.method==='POST'){
    if(!sameOrigin(request))return json({ok:false,error:'origin_mismatch'},403);
    if(mode==='invalid')return new Response(null,{status:303,headers:secureHeaders({location:loginAction,'set-cookie':clearCookie()})});
    const location=(mode==='access'||(mode==='hybrid'&&hasAccessPrincipal(request)))?accessLogoutLocation(request):loginAction;
