...
(function(){
  ...
  function todayActive(){var v=document.body.dataset.crmOwnerView;return !v||v==='today'||document.body.classList.contains('crm-owner-view-today')}
  function install(){if(document.getElementById('crmTodayActionPanel'))return; var base=document.getElementById('crmTodayDashboard'); if(base){base.insertAdjacentHTML('afterend',panel());}else{var target=document.querySelector('main')||document.querySelector('#app')||document.querySelector('.container')||document.body; target.insertAdjacentHTML('afterbegin',panel());} if(todayActive())loadActions();}
  ...
  document.addEventListener('crm:owner-view-change',function(e){if(e.detail?.view==='today')loadActions()});
  document.addEventListener('DOMContentLoaded',install); setTimeout(install,1000); setInterval(function(){if(todayActive())loadActions()},120000);
})();
</script>`;
