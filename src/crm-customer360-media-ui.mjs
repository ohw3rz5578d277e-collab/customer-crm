...
document.addEventListener('click',function(e){var x=e.target&&e.target.closest?e.target.closest('[data-open],[data-direct-customer]'):null;var id=text(x&&((x.dataset&&x.dataset.open)||(x.dataset&&x.dataset.directCustomer)));if(/^\d{8}$/.test(id))setTimeout(function(){load(id)},0)},true);
})();</script>`;
