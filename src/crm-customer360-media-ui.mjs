...
async function load(id){id=text(id);if(!/^\d{8}$/.test(id))return;currentId=id;selectedAvatar='';try{var d=await api('/api/customer360/media/'+id);if(id===currentId)render(id,d.media||{})}catch(e){/* Existing Customer360 detail stays usable if media schema is not ready. */}}
...
document.addEventListener('click',function(e){var x=e.target&&e.target.closest?e.target.closest('[data-open],[data-direct-customer]'):null;var id=text(x&&((x.dataset&&x.dataset.open)||(x.dataset&&x.dataset.directCustomer)));if(/^\d{8}$/.test(id)){setTimeout(function(){load(id)},120);setTimeout(function(){if(currentId===id&&!document.getElementById('crmCustomerMediaCard'))load(id)},500)}},true);
})();</script>`;
