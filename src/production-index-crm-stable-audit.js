...
// build: customer-crm-stable-audit-20260614-01
...
const BUILD = "customer-crm-stable-audit-20260614-01";
...
async function ensureStableSchema(env){
  ...
  await safeRun(env, `CREATE INDEX IF NOT EXISTS idx_stable_crm_reservation_drafts_customer ON crm_reservation_drafts(customer_id, status)`);
  await safeRun(env, `CREATE INDEX IF NOT EXISTS idx_stable_line_logs_status ON customer_line_draft_logs(status, created_at)`);
  await safeRun(env, `CREATE INDEX IF NOT EXISTS idx_stable_follow_tasks_customer ON crm_follow_tasks(customer_id, status)`);
  await safeRun(env, `CREATE INDEX IF NOT EXISTS idx_stable_inquiry_status ON crm_inquiry_pipeline(status, created_at)`);
  await safeRun(env, `CREATE INDEX IF NOT EXISTS idx_stable_delivery_customer ON crm_delivery_progress(customer_id, status)`);
}
...
async function healthCheck(env){
  ...
  const required = [
    "customers", "crm_reservation_drafts", "customer_line_draft_logs", "crm_follow_tasks",
    "crm_delivery_progress", "crm_inquiry_pipeline", "crm_line_ops_logs", "crm_list_workbench_runs",
    "crm_list_workbench_logs", "crm_customer_smart_action_logs"
  ];
  const rows = [];
  for(const name of required){
    const hit = await safeAll(env, `SELECT name FROM sqlite_master WHERE type='table' AND name='${name.replace(/'/g,"''")}' LIMIT 1`);
    rows.push({table:name, ok:!!(hit && hit.results && hit.results.length)});
  }
  const dbOk = !!env.DB;
  return json({
    ok: dbOk && rows.every(r=>r.ok || r.table === "customers"),
    build: BUILD,
    checked_at: new Date().toISOString(),
    bindings: { DB: dbOk, LINE_SERVICE: !!env.LINE_SERVICE, RESERVATION_SERVICE: !!env.RESERVATION_SERVICE },
    tables: rows,
    note: "customers が false の場合は本体データベース未作成またはD1接続先違いです。"
  });
}
...
function injectStableAuditUi(html){
  ...
  const style = `<style id="crm-stable-audit-style">
.crm-stable-audit-btn{position:fixed!important;right:18px!important;bottom:176px!important;z-index:2147482500!important;min-height:44px!important;padding:0 14px!important;border:0!important;border-radius:999px!important;background:#07111f!important;color:#fff!important;font-weight:950!important;box-shadow:0 18px 35px rgba(15,23,42,.25)!important;cursor:pointer!important}
.crm-stable-audit-panel{position:fixed!important;right:18px!important;bottom:230px!important;width:min(520px,calc(100vw - 36px))!important;max-height:70vh!important;overflow:auto!important;background:#fff!important;border:1px solid #dbe5ef!important;border-radius:22px!important;box-shadow:0 24px 60px rgba(15,23,42,.22)!important;z-index:2147482600!important;padding:18px!important;display:none!important}
.crm-stable-audit-panel.open{display:block!important}.crm-stable-audit-panel h3{margin:0 0 10px!important;font-size:18px!important}.crm-stable-audit-panel pre{white-space:pre-wrap!important;background:#f8fafc!important;border:1px solid #e2e8f0!important;border-radius:14px!important;padding:12px!important;font-size:12px!important;line-height:1.6!important}.crm-stable-audit-close{float:right;border:0!important;background:#f1f5f9!important;border-radius:999px!important;width:34px!important;height:34px!important;font-weight:950!important;cursor:pointer!important}
@media(max-width:767px){.crm-stable-audit-btn{right:12px!important;bottom:154px!important}.crm-stable-audit-panel{inset:10px!important;width:auto!important;max-height:calc(100dvh - 20px)!important;border-radius:18px!important}}
</style>`;

  const script = `<script id="crm-stable-audit-script">
(()=>{if(window.__crmStableAudit)return;window.__crmStableAudit=1;
function make(){
 if(document.getElementById('crmStableAuditBtn'))return;
 const btn=document.createElement('button');btn.id='crmStableAuditBtn';btn.className='crm-stable-audit-btn';btn.type='button';btn.textContent='状態確認';
 const panel=document.createElement('div');panel.id='crmStableAuditPanel';panel.className='crm-stable-audit-panel';panel.innerHTML='<button class="crm-stable-audit-close" type="button">×</button><h3>CRM状態確認</h3><p>主要テーブル・連携設定を確認します。</p><pre>未確認</pre>';
 panel.querySelector('.crm-stable-audit-close').onclick=()=>panel.classList.remove('open');
 btn.onclick=async()=>{panel.classList.add('open');const pre=panel.querySelector('pre');pre.textContent='確認中...';try{const r=await fetch('/api/crm-health-check',{cache:'no-store'});pre.textContent=JSON.stringify(await r.json(),null,2);}catch(e){pre.textContent='状態確認に失敗しました: '+(e&&e.message||e);}};
 document.body.appendChild(btn);document.body.appendChild(panel);
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',make);else make();
new MutationObserver(make).observe(document.documentElement,{childList:true,subtree:true});
})();
</script>`;
