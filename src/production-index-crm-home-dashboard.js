...
async function load(){if(!todayActive())return;const [today,line,alerts,delivery,forecast,marketing]=await Promise.all([get('/api/today-dashboard'),get('/api/line-ops/dashboard'),get('/api/priority-alerts'),get('/api/delivery-dashboard'),get('/api/marketing-revenue-forecast'),get('/api/marketing-candidates?segment=all&limit=20')]);if(today?.__http_error||today?.ok===false){const status=today?.status||today?.data?.status||'';$('#crmHomeCards').innerHTML=card('今日やること','読み込み不可','データ障害を確認してください'+(status?' / HTTP '+status:''),'alert');$('#crmHomeTasks').innerHTML='<div class="crm-home-task"><div><b>今日の優先順位を判定できません</b><br><span>空データとして扱わず、復旧後に再確認してください。</span></div><button class="crm-home-open" data-open="今日">再確認</button></div>';$('#crmHomeTasks [data-open]').forEach(b=>b.onclick=()=>openBy(b.dataset.open));return}const todo=today?.summary||today||{};const l=line?.summary||line||{};const a=alerts?.summary||alerts||{};const d=delivery?.summary||delivery||{};const f=forecast||{};const m=Array.isArray(marketing?.items)?marketing.items:[];$('#crmHomeCards').innerHTML=[card('問い合わせ',todo.inquiry_waiting||a.inquiry||0,'返信・日程調整・仮予約',Number(todo.inquiry_waiting||a.inquiry||0)>0?'alert':''),card('LINE未送信',l.unsent||todo.line_unsent||0,'送る前の文面',Number(l.unsent||todo.line_unsent||0)>0?'warn':''),card('予約要確認',todo.reservation_alerts||a.reservation||0,'連携停止・再同期',Number(todo.reservation_alerts||a.reservation||0)>0?'alert':''),card('納品未完了',d.incomplete||a.delivery||0,'納品進捗を確認',Number(d.incomplete||a.delivery||0)>0?'warn':''),card('マーケ候補',m.length||a.marketing||0,'リピート・休眠・VIP',Number(m.length||a.marketing||0)>0?'good':''),card('見込み売上',yen((f.confirmed||0)+(f.pipeline||0)),'確定＋見込み')].join('');const tasks=[];if(Number(todo.inquiry_waiting||a.inquiry||0)>0)tasks.push(task('問い合わせを先に対応','返信待ち・仮予約を確認','問い合わせ'));if(Number(l.unsent||todo.line_unsent||0)>0)tasks.push(task('未送信LINEを送る','作成済み文面を送信済みにする','LINE運用'));if(Number(todo.reservation_alerts||a.reservation||0)>0)tasks.push(task('予約連携を確認','送信漏れ・再同期を確認','予約連携'));if(Number(d.incomplete||a.delivery||0)>0)tasks.push(task('納品進捗を確認','遅延や未完了を確認','納品進捗'));if(m.length)tasks.push(task('マーケ候補へ提案','リピート/休眠候補を確認','マーケDB'));$('#crmHomeTasks').innerHTML=(tasks.length?tasks.join(''):'<div class="crm-home-task"><div><b>今すぐの高優先タスクはありません</b><br><span>必要に応じてCRMメニューから確認してください。</span></div><button class="crm-home-open" data-open="CRMメニュー">メニュー</button></div>');$$('#crmHomeTasks [data-open]').forEach(b=>b.onclick=()=>openBy(b.dataset.open))}
window.__crmHomeDashboard={mount,load};document.addEventListener('crm:owner-view-change',e=>{if(e.detail?.view==='today')load()});document.readyState==='loading'?document.addEventListener('DOMContentLoaded',mount):mount();})();
</script>`;
