...
export function injectOwnerLogoutPostRoute(html){
  ...
  const script=`<script id="crm-owner-logout-post-route">(()=>{if(window.__crmOwnerLogoutPostRoute)return;window.__crmOwnerLogoutPostRoute=1;document.addEventListener('click',e=>{const target=e.target&&e.target.closest?e.target.closest('[data-crm-logout],[data-act="logout"],.crm-logout-btn,button,a'):null;if(!target)return;const text=(target.textContent||'').trim();if(!target.matches('[data-crm-logout],[data-act="logout"],.crm-logout-btn')&&text!=='ログアウト')return;e.preventDefault();e.stopImmediatePropagation();try{localStorage.clear();sessionStorage.clear()}catch(_){}const base=typeof window.__CRM_BASE_PATH__==='string'?window.__CRM_BASE_PATH__.replace(/\/$/,''):'';const form=document.createElement('form');form.method='POST';form.action=base+'/__crm/owner-logout';form.style.display='none';document.body.appendChild(form);form.submit()},true)})()<\/script>`;
