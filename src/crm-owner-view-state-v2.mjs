...
function settleCustomerViewport(focusSearch=false){let tries=0;const settle=()=>{const list=$('crmMktList');if(!list){if(++tries<20)setTimeout(settle,25);return false}const top=$('crmOwnerWorkspaceHeader')||$('crmOwnerAppShell')||list;top.scrollIntoView({block:'start'});if(focusSearch){const input=$('crmGlobalSearch');if(!input){if(++tries<20)setTimeout(settle,25);return false}input.focus({preventScroll:true});input.scrollIntoView({bloc:'center'})}return true};if(!settle())requestAnimationFrame(()=>requestAnimationFrame(settle))}
