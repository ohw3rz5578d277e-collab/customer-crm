...
.crm-owner-shell-v2 #crmOwnerStatusSheet.open{display:block!important;position:fixed!important;top:72px!important;right:24px!important;left:auto!important;bottom:auto!important;width:min(520px,calc(100vw - 48px))!important;max-height:calc(100vh - 96px)!important;overflow:auto!important;z-index:2147485200!important;background:#f6f9fa!important;border:1px solid var(--crm-shell-line)!important;border-radius:18px!important;box-shadow:0 24px 70px rgba(15,23,42,.22)!important;padding:16px!important;box-sizing:border-box!important;color:var(--crm-shell-text)!important}
...
@media(max-width:767px){
 ...
 #crmOwnerMobileNav{position:fixed!important;left:0!important;right:0!important;bottom:0!important;z-index:2147485000!important;display:grid!important;grid-template-columns:repeat(5,minmax(0,1fr))!important;height:calc(70px + env(safe-area-inset-bottom))!important;padding:5px 6px calc(5px + env(safe-area-inset-bottom))!important;box-sizing:border-box!important;background:rgba(255,255,255,.98)!important;border-top:1px solid var(--crm-shell-line)!important;box-shadow:0 -8px 28px rgba(15,23,42,.08)!important}
...
const navObserver=new MutationObserver(records=>{for(const r of records){for(const n of r.addedNodes){if(n?.nodeType===1&&(n.id==='crmOwnerMobileNav'||n.querySelector?.('#crmOwnerMobileNav'))){canonicalizeMobileNav();return}}}});if(document.documentElement)navObserver.observe(document.documentElement,{childList:true,subtree:true});
