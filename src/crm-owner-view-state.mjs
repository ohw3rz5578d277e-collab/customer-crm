...
body[data-crm-owner-view="customers"] [data-crm-owner-operational="1"],body[data-crm-owner-view="search"] [data-crm-owner-operational="1"]{display:none!important}
...
const OPERATIONAL_IDS=['crmTodayDashboard','crmReservationStatus','crmDeliveryDeadlinePanel','crmLegacyOperations','crmTodayFilterPanel'];
...
function markOperational(){for(const id of OPERATIONAL_IDS){const el=$(id);if(el)el.setAttribute('data-crm-owner-operational','1')}document.querySelectorAll('.crm-today-dash,.crm-delivery-panel,[data-crm-owner-today],[data-crm-owner-operational-section]').forEach(el=>{if(!el.closest('#crmMktList,#crmMktHome,#crmMktNav'))el.setAttribute('data-crm-owner-operational','1')})}
...
function setView(name){markHost();markOperational();document.body.dataset.crmOwnerView=name;document.body.classList.toggle('crm-owner-view-today',name==='today');setActive(name);return name}
function settleCustomerViewport(focusSearch=false){let tries=0;const settle=()=>{markHost();markOperational();const heading=$('crmOwnerCustomerTitle')||$('crmMktList')?.querySelector('h2,.crm-search-head')||$('crmMktList');if(heading){heading.scrollIntoView({block:'start'});if(focusSearch){const input=$('crmGlobalSearch');if(input){input.focus({preventScroll:true});input.scrollIntoView({block:'center'});return true}}else return true}if(++tries<20)setTimeout(settle,25);return false};if(!settle())requestAnimationFrame(()=>requestAnimationFrame(settle))}
...
function boot(){window.__crmOwnerView=api;markHost();markOperational();bindNav();if(mobile())api.showToday({scroll:false});new MutationObserver(()=>{markHost();markOperational();bindNav()}).observe(document.documentElement,{childList:true,subtree:true});window.addEventListener('resize',()=>{markHost();markOperational();bindNav()},{passive:true})}
