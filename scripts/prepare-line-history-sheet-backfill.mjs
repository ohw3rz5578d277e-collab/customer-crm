...
resolved.candidates=markBatchDuplicates(resolved.candidates);
...
const summary={generated_at:new Date().toISOString(),planner_version:4,source:{line_log:path.basename(linePath),customer_master:path.basename(masterPath)},line_log_rows:lineLog.row_count,parsed_message_rows:lineLog.rows.length,customer_master_rows:master.row_count,customer_master_usable_entries:master.entries.length,candidate_rows:resolved.candidates.length,batch_duplicate_rows:resolved.candidates.filter(x=>x.batch_duplicate).length,planned_insert_candidate_rows:resolved.candidates.filter(x=>!x.batch_duplicate).length,remote_line_user_resolution_rows:resolved.candidates.filter(x=>!x.customer_id_hint&&x.line_user_id).length,legacy_customer_id_ignored_rows:resolved.warnings.filter(x=>x.reason==='legacy_customer_id_ignored').length,skipped_rows:resolved.skipped.length,skipped_reasons:countReasons(resolved.skipped),warning_reasons:countReasons(resolved.warnings),match_sources:countMatchSources(resolved.candidates),conflict_rows:resolved.conflicts.length,safety:{production_d1_write:0,customer_id_generation:0,customer_update:0,customer_delete:0,line_send:0,remote_identity_requires_unique_match:true,stale_customer_id_fallback_requires_nonexistent_hint:true,batch_duplicate_candidates_skipped:true,commit_requires_owner_approval:true},files:{preview_sql:previewPath,commit_sql:commitPath}};
