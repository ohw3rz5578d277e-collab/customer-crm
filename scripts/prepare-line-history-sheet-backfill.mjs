...
function identityCondition(alias='c',prefix='candidates.'){
  return `((${prefix}customer_id_hint<>'' AND ${alias}.customer_id=${prefix}customer_id_hint AND (${prefix}line_user_id='' OR COALESCE(${alias}.line_user_id,'')=${prefix}line_user_id)) OR (${prefix}customer_id_hint='' AND ${prefix}line_user_id<>'' AND COALESCE(${alias}.line_user_id,'')=${prefix}line_user_id))`;
}
function literalIdentityCondition(x,alias='c'){
  if(x.customer_id_hint&&x.line_user_id)return `${alias}.customer_id=${sqlText(x.customer_id_hint)} AND COALESCE(${alias}.line_user_id,'')=${sqlText(x.line_user_id)}`;
...
const preview=['-- READ ONLY. Generated by prepare-line-history-sheet-backfill.mjs','-- Resolves legacy C... identities by current customers.line_user_id only when exactly one current CRM customer matches.','-- This file must not contain INSERT/UPDATE/DELETE/CREATE/ALTER/DROP.'];
...
const commit=['-- WRITE FILE. DO NOT RUN WITHOUT FRESH OWNER APPROVAL.','-- INSERT ONLY. Current CRM identity must resolve to exactly one customer by Customer ID and/or LINE UserID.','-- No customer creation/update/delete. Idempotent by message_key + exact fingerprint.'];
